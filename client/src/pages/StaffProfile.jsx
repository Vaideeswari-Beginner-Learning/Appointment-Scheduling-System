...
const StaffProfile = () => {
    ...
    if (error || !profile) return (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', flexDirection: 'column', gap: '16px' }}>
            ...
            <button onClick={() => navigate(-1)} style={{ background: '#4F46E5', color: 'white', border: 'none', borderRadius: '10px', padding: '10px 24px', cursor: 'pointer', fontWeight: 700 }}>Go Back</button>
        </div>
    );
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
            <div style={{ maxWidth: '760px', margin: '0 auto', padding: '40px 20px' }}>
                {/* Back */}
                <button onClick={() => navigate(-1)} style={{ background: 'rgba(255,255,255,0.2)', color: 'white', border: 'none', borderRadius: '10px', padding: '10px 20px', cursor: 'pointer', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px', backdropFilter: 'blur(10px)' }}>
                ...
                <div style={{ background: 'white', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 25px 50px rgba(0,0,0,0.15)', marginBottom: '24px' }}>
                    ...
                    <div style={{ padding: '0 32px 32px', marginTop: '-50px' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
                            <div>
                                <h1 style={{ margin: '0 0 4px', fontSize: '28px', fontWeight: 900, color: '#0F172A' }}>{profile.name}</h1>
                            ...
                            {/* Availability badge */}
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', background: `${statusColors[profile.availabilityStatus] || '#94A3B8'}20`, borderRadius: '99px', border: `1px solid ${statusColors[profile.availabilityStatus] || '#94A3B8'}40` }}>
                                <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: statusColors[profile.availabilityStatus] || '#94A3B8' }} />
                                <span style={{ fontSize: '13px', fontWeight: 800, color: statusColors[profile.availabilityStatus] || '#94A3B8', textTransform: 'capitalize' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', margin: '28px 0' }}>
                            {[
                                ...
                                { icon: Star, label: 'Avg Rating', value: profile.averageRating ? `${profile.averageRating}/5` : 'N/A', color: '#D97706', bg: '#FEF3C7' },
                            ].map((stat, i) => (
                                <div key={i} style={{ padding: '20px', background: stat.bg, borderRadius: '16px', textAlign: 'center' }}>
                                    ...
                                    <div style={{ fontSize: '22px', fontWeight: 900, color: '#0F172A' }}>{stat.value}</div>
                ...
                <div style={{ background: 'white', borderRadius: '24px', padding: '28px', boxShadow: '0 8px 20px rgba(0,0,0,0.08)' }}>
                    <h2 style={{ margin: '0 0 20px', fontSize: '20px', fontWeight: 900, color: '#0F172A' }}>
                            ...
                            {profile.reviews.map((review, i) => (
                                <div key={i} style={{ padding: '16px 20px', background: '#F8FAFC', borderRadius: '14px', border: '1px solid #E2E8F0' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                                        <div>
                                            <div style={{ fontWeight: 800, fontSize: '14px', color: '#0F172A' }}>{review.patientName || 'Anonymous'}</div>
                                            <div style={{ fontSize: '12px', color: '#94A3B8', marginTop: '2px' }}>{new Date(review.createdAt).toLocaleDateString()}</div>
                                        </div>
